...
window.addEventListener('message', event => {
  ...
  if (data.type === 'retro-pong:set-muted') {
...
function drawGame() {
  ...
  drawText('[SPACE] PAUSE', BASE_W / 2,     14, 11, '#333333');
}
...
function drawPaused() {
  ...
  drawText('PRESS SPACE TO RESUME', BASE_W / 2, BASE_H / 2 + 30, 18, COLORS.dimmed);
  drawText('E → MAIN MENU',   BASE_W / 2, BASE_H / 2 + 60, 14, COLORS.dimmed);
}
...
function drawWon() {
  ...
  drawText('E → MAIN MENU',             BASE_W / 2, BASE_H / 2 + 185, 13, COLORS.dimmed);
}
...
function drawDifficultyMenu() {
  ...
  blinkText('W/S + ENTER TO SELECT', BASE_W / 2, BASE_H - 60, 12);
  drawText('E → BACK', BASE_W / 2, BASE_H - 35, 11, '#444');
}
